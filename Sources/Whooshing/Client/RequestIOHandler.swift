...
final class RequestHandler: ChannelDuplexHandler, Sendable {
    ...
    func channelRead(context: ChannelHandlerContext, data: NIOAny) {
        ...
        if let ioHandler = self.ioHandler {
            do {
                var res = try ioHandler.get(response: buffer, context: context)
    ...
    func write(context: ChannelHandlerContext, data: NIOAny, promise: EventLoopPromise<Void>?) {
        ...
        if let ioHandler = self.ioHandler {
            do {
                let req = try ioHandler.send(request: buffer, context: context, allocator: byteBufferAllocator)
